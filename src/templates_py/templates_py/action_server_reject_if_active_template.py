    ...
    def goal_callback(self, goal_request):
        with self.goal_lock:
            if self.goal_handle is not None and self.goal_handle.is_active:
                self.get_logger().warn('Goal is currently active. Incoming goal rejected')
...
def main(args=None):
    ...
    action_server_reject_if_active_template = ActionServerTemplate('action_server_reject_if_active_template')
    rclpy.spin(action_server_reject_if_active_template, executor=MultiThreadedExecutor())
    rclpy.shutdown()
