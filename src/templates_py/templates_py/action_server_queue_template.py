    ...
    def handle_accepted_callback(self, goal_handle):
        with self.goal_queue_lock:
            if self.current_goal is not None:
                ...
                self.get_logger().info('Incoming goal is in the queue')
            else:
...
def main(args=None):
    ...
    action_server_queue_template = ActionServerTemplate('action_server_queue_template')
    rclpy.spin(action_server_queue_template, executor=MultiThreadedExecutor())
    rclpy.shutdown()
